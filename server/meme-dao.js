...
exports.listMemes = () => {
  return new Promise((resolve, reject) => {
    const sql =
      "SELECT * FROM meme INNER JOIN field on meme.id = field.memeId INNER JOIN user on meme.user_id = user.user_id";
    db.all(sql, [], (err, rows) => {
      if (err) {
        reject(err);
        return;
...
exports.listPublicMemes = () => {
  return new Promise((resolve, reject) => {
    const sql =
      "SELECT * FROM meme INNER JOIN field on meme.id = field.memeId INNER JOIN user on meme.user_id = user.user_id WHERE visibility = ?";
